const axios = require("axios");


const ACCESS_TOKEN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..mOFcWpCJJImTagMmIyayNFygkKRqZMnneVe6NMIC17o";

const log = async (stack, level, pkg, message) => {
  try {
    console.log("Attempting to send log:", { stack, level, pkg, message }); 
    const response = await axios.post(
      "http://20.244.56.144/evaluation-service/logs",
      {
        stack,
        level,
        package: pkg,
        message,
      },
      {
        headers: {
          Authorization: `Bearer ${ACCESS_TOKEN}`,
          "Content-Type": "application/json",
        },
      }
    );
    console.log("Log sent successfully:", response.data); // success line
  } catch (err) {
    console.error("Failed to log:", err.response?.data || err.message); // show backend error
  }
};
